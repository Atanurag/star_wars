...
const InternalDetail: FC = () => {
  const { category_type, id_type } = useParams();
  ...
  useEffect(() => {
    if (category_type && id_type) {
      setLoading(true);
      fetch(`https://www.swapi.tech/api/${category_type}/${id_type}`)
        .then((res) => res.json())
        .then((json) => {
  ...
  return (
    <>
      ...
      >
        ...
        {category_type == 'people' && <Card shadow="sm" padding="lg" radius="md" withBorder style={{ flex: '1 1 350px', maxWidth: 400 }}>
          <Card.Section>
            <Image
          ...
          <Stack gap={6}>
            ...
            <Text size="sm">
              ...
              <span style={{ cursor: 'pointer' }} onClick={() => {
                ...
                const baseCategory = currentPathParts[1];
                const baseId = currentPathParts[2];

                const newPath = `/detail-page/${baseCategory}/${baseId}/internal-detail/${categoryToView}/${categoryId}`;
                navigate(newPath);


                // const path = `${categoryToView}/${categoryId}`;
                // navigate(path)
                ;
              }}> View Home</span>
            </Text>
